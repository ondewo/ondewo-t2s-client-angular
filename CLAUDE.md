...
```text
Release ONDEWO T2S Angular Client ${ONDEWO_T2S_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO T2S Angular Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO T2S Angular Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/t2s-client-angular`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/t2s-client-angular versions --json
```
